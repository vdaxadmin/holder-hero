  ...
  const renderObstacle = (obs: Obstacle, index: number) => {
    ...
    if (obs.type === 'fud') {
      return (
        <g key={`obs-${index}`}>
          ...
          <text x={baseX + OBSTACLE_WIDTH/2} y={22} textAnchor="middle" fontSize="10" fill="white" fontWeight="bold">📰 BREAKING</text>
          <text x={baseX + OBSTACLE_WIDTH/2} y={50} textAnchor="middle" fontSize="9" fill="#ff6b6b" fontWeight="bold">
            {obs.headline?.slice(0, 15) || 'FUD NEWS!'}
          </text>
          <text x={baseX + OBSTACLE_WIDTH/2} y={gapY - 20} textAnchor="middle" fontSize="24">📉</text>
          ...
          <text x={baseX + OBSTACLE_WIDTH/2} y={gapEnd + 22} textAnchor="middle" fontSize="10" fill="white" fontWeight="bold">⚠️ ALERT</text>
          <text x={baseX + OBSTACLE_WIDTH/2} y={gapEnd + 50} textAnchor="middle" fontSize="20">😱</text>
          <text x={baseX + OBSTACLE_WIDTH/2} y={gapY + gapSize/2 + 5} textAnchor="middle" fontSize="12" fill="#ff6b6b" opacity={0.7}>FUD</text>
        </g>
      );
    }
    ...
    if (obs.type === 'margin') {
      ...
      return (
        <g key={`obs-${index}`}>
          ...
          <text x={baseX + OBSTACLE_WIDTH/2} y={gapY - 42} textAnchor="middle" fontSize="28">💣</text>
          ...
          <text x={baseX + OBSTACLE_WIDTH/2} y={gapY - 10} textAnchor="middle" fontSize="10" fill="#ff4757" fontWeight="bold">-50% VỐN!</text>
          <rect x={baseX} y={gapEnd} width={OBSTACLE_WIDTH} height={GAME_HEIGHT - gapEnd} fill="#2d1f1f" rx={8} />
          <text x={baseX + OBSTACLE_WIDTH/2} y={gapEnd + 58} textAnchor="middle" fontSize="28">💥</text>
          <text x={baseX + OBSTACLE_WIDTH/2} y={gapEnd + 102} textAnchor="middle" fontSize="10" fill="#ff4757">CALL!</text>
        ...
        return (
          <g key={`obs-${index}`}>
            ...
            <g transform={`translate(${baseX + OBSTACLE_WIDTH/2}, ${gapY - 60 + whaleY})`}>
              <text textAnchor="middle" fontSize="40" y={15}>🐋</text>
            </g>
            <text x={baseX + OBSTACLE_WIDTH/2} y={42} textAnchor="middle" fontSize="10" fill="#7c7cf7">ALERT!</text>
            <rect x={baseX} y={gapEnd} width={OBSTACLE_WIDTH} height={GAME_HEIGHT - gapEnd} fill="#1a3a5c" rx={8} />
            <text x={baseX + OBSTACLE_WIDTH/2} y={gapEnd + 60} textAnchor="middle" fontSize="30">🌊</text>
    ...
    if (obs.type === 'fomo') {
        ...
        return (
          <g key={`obs-${index}`}>
            ...
            <text x={baseX + OBSTACLE_WIDTH/2} y={50} textAnchor="middle" fontSize="14" fill="#fff" fontWeight="bold">FOMO</text>
            <text x={baseX + OBSTACLE_WIDTH/2} y={gapY - 50} textAnchor="middle" fontSize="24">🚀</text>
            <rect x={baseX} y={gapEnd} width={OBSTACLE_WIDTH} height={GAME_HEIGHT - gapEnd} fill={`rgba(255,${100 + flameIntensity * 100},0,0.9)`} rx={8} />
            <text x={baseX + OBSTACLE_WIDTH/2} y={gapEnd + 90} textAnchor="middle" fontSize="20">🦍</text>
            <rect x={baseX} y={gapY} width={OBSTACLE_WIDTH} height={gapSize} fill={`rgba(255,165,2,${0.1 + flameIntensity * 0.1})`} />
            <text x={baseX + OBSTACLE_WIDTH/2} y={gapY + gapSize/2} textAnchor="middle" fontSize="10" fill="#ffa502" opacity={0.8}>HOT ZONE</text>
          </g>
        );
    }
    ...
    if (obs.type === 'ice') {
        return (
          <g key={`obs-${index}`}>
            ...
            <text x={baseX + OBSTACLE_WIDTH/2} y={40} textAnchor="middle" fontSize="12" fill="#00d4ff" fontWeight="bold">ĐÓNG</text>
            <text x={baseX + OBSTACLE_WIDTH/2} y={gapY - 30} textAnchor="middle" fontSize="30">❄️</text>
    ...
    return (
      <g key={`obs-${index}`}>
        ...
        <text x={baseX + OBSTACLE_WIDTH/2} y={gapY - 20} textAnchor="middle" fontSize="24">{obs.emoji}</text>
        <rect x={baseX} y={gapEnd} width={OBSTACLE_WIDTH} height={GAME_HEIGHT - gapEnd} fill={obs.color} rx={8} />
        <text x={baseX + OBSTACLE_WIDTH/2} y={gapEnd + 30} textAnchor="middle" fontSize="24">{obs.emoji}</text>
      </g>
    );
  };
        ...
        <svg width="100%" height="100%" viewBox={`0 0 ${GAME_WIDTH} ${GAME_HEIGHT}`} preserveAspectRatio="xMidYMid slice">
          ...
          {candles.map((candle, i) => {
            ...
            return (
              <g key={`candle-${i}`}>
                ...
                <text x={candle.x + CANDLE_WIDTH / 2} y={animatedY - candle.wickTop - 24} textAnchor="middle" fontSize="14" fill={changeColor} fontWeight="bold" style={{ textShadow: '0 0 5px black' }}>{candle.ticker}</text>
                <text x={candle.x + CANDLE_WIDTH / 2} y={animatedY - candle.wickTop - 8} textAnchor="middle" fontSize="11" fill={changeColor} fontWeight="bold" opacity={0.9}>{candle.isGreen ? '+' : ''}{candle.priceChange}%</text>
              </g>
            );
          })}
          ...
          {powerups.map((p, i) => (
            <g key={`powerup-${i}`}>
              ...
              <text x={p.x} y={p.y + 8} textAnchor="middle" fontSize="30">{p.emoji}</text>
            </g>
          ))}
          ...
          <g transform={`translate(80, ${playerY})`}>
            ...
            <text x={-5} y={PLAYER_SIZE / 2 + 5} fontSize="20">💵</text>
            <text x={PLAYER_SIZE - 5} y={PLAYER_SIZE / 2 + 5} fontSize="20" style={{ transform: 'scaleX(-1)', transformOrigin: 'center' }}>💵</text>
            ...
            <text x={PLAYER_SIZE / 2} y={PLAYER_SIZE / 2 + 10} textAnchor="middle" fontSize="28">{isFrozen ? '🥶' : playerExpression}</text>
            <text x={PLAYER_SIZE / 2} y={0} textAnchor="middle" fontSize="20">🎩</text>
          </g>
          
          {/* Floating Text */}
          {floatingTexts.map((t) => <text key={t.id} x={t.x} y={t.y} fill={t.color} fontSize="18" fontWeight="bold" opacity={t.life / 60} style={{ textShadow: '0 0 10px black' }}>{t.text}</text>)}
          ...
          {buffettActive && !buffettCollected && (
            <g transform={`translate(${buffettX}, 60)`}>
              ...
              <text x={15} y={45} fontSize="50">🧓</text>
              <rect x={60} y={0} width={120} height={35} fill="rgba(255,255,255,0.95)" rx={10} />
              <text x={70} y={22} fontSize="11" fill="#333" fontWeight="bold">"Be greedy!"</text>
            </g>
          )}
          ...
          {activeMarketMover && marketMoverTimer > 0 && activeMarketMover.currentQuote && (
             <g>
              ...
              <g transform={`translate(${GAME_WIDTH / 2 - 175}, 140)`}>
                ...
                <text x={40} y={43} textAnchor="middle" fontSize="28">{activeMarketMover.avatar}</text>
                <text x={75} y={25} fill="white" fontSize="14" fontWeight="bold">{activeMarketMover.name}</text>
                <text x={75} y={42} fill="#888" fontSize="12">@{activeMarketMover.id}</text>
                <text x={75} y={70} fill="white" fontSize="16" fontWeight="bold">"{activeMarketMover.currentQuote.text}"</text>
                <rect x={255} y={10} width={85} height={28} fill={MARKET_EFFECTS[activeMarketMover.currentQuote.effect].color} rx={14} />
                <text x={297} y={30} textAnchor="middle" fill="#0a1628" fontSize="12" fontWeight="bold">
          ...
          {bossCallActive && (
            <g>
              ...
              <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 - 90} textAnchor="middle" fontSize="50">😠</text>
              <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 - 20} textAnchor="middle" fill="white" fontSize="24" fontWeight="bold">SẾP GỌI!</text>
              <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 + 50} textAnchor="middle" fill={bossCallTimer <= 2 ? '#ff4757' : '#ffd700'} fontSize="40" fontWeight="bold">{bossCallTimer}s</text>
              ...
              <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 + 130} textAnchor="middle" fill="#00ff88" fontSize="16" fontWeight="bold">TAP NHANH ĐỂ ẨN APP! 📱</text>
            </g>
          )}
          ...
          <g>
                  ...
                  {marketIndices.map((idx, i) => {
                    ...
                    return (
                      <g key={`${loop}-${i}`} transform={`translate(${i * 130}, 0)`}>
                        <text x={10} y={14} fontSize="11" fill="#888">{idx.emoji}</text>
                        <text x={25} y={14} fontSize="12" fill="#fff" fontWeight="bold">{idx.symbol}</text>
                        <text x={25} y={27} fontSize="12" fill={color} fontWeight="bold">{idx.value >= 1000 ? idx.value.toFixed(0) : idx.value.toFixed(2)}</text>
                        <text x={90} y={27} fontSize="11" fill={color}>{idx.change >= 0 ? '▲' : '▼'}{Math.abs(idx.change).toFixed(2)}%</text>
                      </g>
                    );
                  })}
                </g>
            ...
            <text x={20} y={68} fill="#888" fontSize="11">ĐIỂM SỐ</text>
            <text x={20} y={92} fill="#00ff88" fontSize="28" fontWeight="bold" filter="url(#glow)">💰 {score.toLocaleString()}</text>
            
             {/* Combo */}
            {combo > 1 && (
              <g>
                <rect x={200} y={45} width={90} height={55} fill="rgba(255,215,0,0.2)" rx={10} stroke="#ffd700" strokeWidth={2} />
                <text x={210} y={68} fill="#ffd700" fontSize="11">COMBO</text>
                <text x={245} y={92} textAnchor="middle" fill="#ffd700" fontSize="26" fontWeight="bold">x{combo.toFixed(1)}</text>
              </g>
            )}
            ...
            <g>
              ...
              <text x={GAME_WIDTH / 2 - 90} y={65} fill="#888" fontSize="11">🇻🇳 VN-INDEX</text>
              <text x={GAME_WIDTH / 2 - 90} y={92} fill={vnIndex.trend === 'up' ? '#00ff88' : '#ff4757'} fontSize="26" fontWeight="bold">{vnIndex.value.toFixed(2)}</text>
              <text x={GAME_WIDTH / 2 + 70} y={92} fill={vnIndex.trend === 'up' ? '#00ff88' : '#ff4757'} fontSize="16" fontWeight="bold">{vnIndex.trend === 'up' ? '▲' : '▼'}{Math.abs(vnIndex.change).toFixed(2)}%</text>
            </g>
            ...
            {/* Sound */}
             <g onClick={(e) => { e.stopPropagation(); onToggleSound(); }} style={{ cursor: 'pointer' }}>
              ...
              <text x={GAME_WIDTH / 2 + 132} y={82} textAnchor="middle" fontSize="26">{soundEnabled ? '🔊' : '🔇'}</text>
            </g>
            ...
            <g>
              ...
              <text x={GAME_WIDTH - 190} y={62} fill="#888" fontSize="10">VỐN HIỆN TẠI</text>
              <text x={GAME_WIDTH - 190} y={82} fill={capital >= startProfit ? '#00ff88' : '#ff4757'} fontSize="18" fontWeight="bold">💰 {(capital / 1000000).toFixed(2)}M</text>
              <text x={GAME_WIDTH - 190} y={96} fill={profit >= 0 ? '#00ff88' : '#ff4757'} fontSize="12">P/L: {profit >= 0 ? '+' : ''}{(profit / 1000000).toFixed(2)}M</text>
            </g>

            {/* Powerups Indicators */}
            <g transform="translate(10, 110)">
              {isInvincible && <text fontSize="26">🛡️</text>}
              {hasSpeedBoost && <text x={35} fontSize="26">☕</text>}
              {hasDoublePoints && <text x={70} fontSize="26">💎</text>}
              {isFrozen && <text x={105} fontSize="26">🥶</text>}
              {extraLives > 0 && <text x={140} fontSize="20" fill="#ffd700">🧓 x{extraLives}</text>}
            </g>
            ...
            {currentQuote && (
                <g>
                    ...
                    <text x={GAME_WIDTH / 2} y={GAME_HEIGHT - 47} textAnchor="middle" fill="white" fontSize="14" fontWeight="bold">{currentQuote}</text>
                </g>
            )}
            ...
            {!gracePeriodActive && (
              <g onClick={(e) => { e.stopPropagation(); onCashout(); }} style={{ cursor: 'pointer' }}>
                ...
                <text x={80} y={GAME_HEIGHT - 50} textAnchor="middle" fill={profit >= 0 ? '#0a1628' : '#fff'} fontSize="14" fontWeight="bold">
            ...
            {gracePeriodActive && (
              <g>
                ...
                <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 - 35} textAnchor="middle" fill="#00ff88" fontSize="28" fontWeight="bold">GET READY!</text>
                <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 - 5} textAnchor="middle" fill="#ffd700" fontSize="18">💰 Vốn: 10,000,000 VND</text>
              </g>
            )}
          </g>
        </svg>

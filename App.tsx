...
import { 
  GAME_WIDTH, GAME_HEIGHT, GRAVITY, JUMP_FORCE, PLAYER_SIZE, OBSTACLE_WIDTH, GAP_SIZE, GAME_SPEED, CANDLE_WIDTH, 
  STARTING_CAPITAL, MIN_CAPITAL, DIFFICULTY, OBSTACLE_REWARDS, POWERUP_BONUSES, VN30_TICKERS, MARKET_INDICES, 
  MARKET_MOVERS, OBSTACLE_TYPES, POWERUP_TYPES, FUD_HEADLINES, QUOTES, SITUATION_QUOTES, MARKET_EFFECTS 
} from './constants';
import { Obstacle, Powerup, Candle, Particle, FloatingText, MarketMover, LeaderboardEntry, GameState } from './types';
...
export default function HodlHeroGame() {
  ...
  const [vnIndex, setVnIndex] = useState({ value: 1250, change: 0.5, trend: 'up' });
  const [marketIndices, setMarketIndices] = useState(MARKET_INDICES);
  const [tickerOffset, setTickerOffset] = useState(0);
  
  // Events
  const [activeMarketMover, setActiveMarketMover] = useState<MarketMover | null>(null);
  ...
  const generateCandles = useCallback(() => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      newCandles.push({
        ...
        price: basePrice,
        priceChange: isGreen ? (1 + Math.random() * 7).toFixed(2) : (-(1 + Math.random() * 7)).toFixed(2),
  ...
  const initGame = useCallback(() => {
    ...
    setFloatingTexts([]);
    setVnIndex({ value: 1200 + Math.random() * 100, change: (Math.random() - 0.5) * 4, trend: Math.random() > 0.5 ? 'up' : 'down' });
    setTickerOffset(0);
  ...
  const checkCollision = (px: number, py: number, obstacle: Obstacle) => {
    ...
    const playerBox = { x: px + 5, y: py + 5, width: PLAYER_SIZE - 15, height: PLAYER_SIZE - 15 };
    const topBox = { x: obstacle.x, y: 0, width: OBSTACLE_WIDTH, height: obstacle.gapY };
    const bottomBox = { x: obstacle.x, y: obstacle.gapY + gapSize, width: OBSTACLE_WIDTH, height: GAME_HEIGHT - obstacle.gapY - gapSize };
    const collidesWith = (a: any, b: any) => a.x < b.x + b.width && a.x + a.width > b.x && a.y < b.y + b.height && a.y + a.height > b.y;
    return collidesWith(playerBox, topBox) || collidesWith(playerBox, bottomBox);
  };
  
  const applyPowerup = (powerup: Powerup) => {
    const bonus = POWERUP_BONUSES[powerup.type] || 100000;
    const bonusText = bonus >= 1000000 ? `+${(bonus / 1000000).toFixed(1)}M` : `+${(bonus / 1000).toFixed(0)}K`;
    
    setFloatingTexts(prev => [...prev, { id: Date.now(), text: `${powerup.effect} ${bonusText}`, x: powerup.x, y: powerup.y, life: 60, color: powerup.color }]);
  ...
  const jump = useCallback(() => {
    if (bossCallActive) {
      ...
      setBossCallProgress(p => {
        ...
        if (newProgress >= 100) {
          ...
          setBossCallActive(false);
          setScore(s => s + 500);
          setFloatingTexts(prev => [...prev, { id: Date.now(), text: '+500 SAFE!', x: GAME_WIDTH / 2, y: GAME_HEIGHT / 2, life: 60, color: '#00ff88' }]);
    ...
    if (gameState === 'playing') {
      ...
      setParticles(prev => [...prev, ...Array(5).fill(null).map(() => ({ x: 80, y: playerY + PLAYER_SIZE / 2, vx: -2 - Math.random() * 2, vy: (Math.random() - 0.5) * 4, life: 20, color: '#ffd700' }))]);
    } else if (gameState === 'menu') {
      startGame();
    }
  }, [gameState, playerY, bossCallActive, soundEnabled]);
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      else if (marketEffect === 'chaos') speed = speed * (0.8 + Math.random() * 0.5);
          ...
          if (!isInvincible && extraLives > 0) {
             ...
             setCapital(c => {
                 ...
                 if (newC <= MIN_CAPITAL) {
                     ...
                     setGameState('gameOver');
                     if (score > highScore) { setHighScore(score); soundManager.newHighScore(); }
                     return 0;
                 }
                 return newC;
             });
      ...
      setPlayerExpression(playerY < GAME_HEIGHT / 3 ? '🤑' : playerY > GAME_HEIGHT * 2 / 3 ? '😱' : '😎');
      ...
      setCandles(prev => {
        ...
        if (Math.max(...updated.map(c => c.x)) < GAME_WIDTH) {
           ...
           updated.push({
             ...
             isGreen,
             baseY: GAME_HEIGHT - 100 - Math.random() * 200,
             ticker: VN30_TICKERS[Math.floor(Math.random() * VN30_TICKERS.length)],
      ...
      if (frameCountRef.current > DIFFICULTY.GRACE_PERIOD) {
         currentDifficultyRef.current = Math.max(DIFFICULTY.MIN_OBSTACLE_INTERVAL, currentDifficultyRef.current * DIFFICULTY.DIFFICULTY_INCREASE_RATE);
         if (frameCountRef.current - lastObstacleRef.current > currentDifficultyRef.current) {
             let availableTypes = OBSTACLE_TYPES.filter(t => score < 500 ? t.type !== 'margin' : true);
             ...
             setObstacles(prev => [...prev, {
                 ...
                 headline: type.type === 'fud' ? FUD_HEADLINES[Math.floor(Math.random() * FUD_HEADLINES.length)] : undefined,
                 bombTimer: type.type === 'margin' ? 0 : undefined,
                 whaleY: type.type === 'whale' ? 0 : undefined,
                 whaleDir: type.type === 'whale' ? (Math.random() > 0.5 ? 1 : -1) : undefined,
      ...
      if (frameCountRef.current - lastPowerupRef.current > DIFFICULTY.POWERUP_INTERVAL && Math.random() < DIFFICULTY.POWERUP_SPAWN_CHANCE) {
          const pt = POWERUP_TYPES[Math.floor(Math.random() * POWERUP_TYPES.length)];
          setPowerups(prev => [...prev, { x: GAME_WIDTH, y: 100 + Math.random() * (GAME_HEIGHT - 200), ...pt }]);
      ...
      setObstacles(prev => prev.map(obs => {
          ...
          if (newObs.type === 'whale' && newObs.whaleY !== undefined) {
             ...
             if (Math.abs(newObs.whaleY) > 30) newObs.whaleDir = -(newObs.whaleDir || 1);
          }
          if (newObs.type === 'margin') newObs.bombTimer = ((newObs.bombTimer || 0) + 1) % 30;
          ...
          if (!newObs.passed && newObs.x + OBSTACLE_WIDTH < 80) {
              ...
              soundManager.pass();
              if (combo < 5) {
                  setCombo(c => { const nc = Math.min(c + 0.5, 5); if (nc > c) soundManager.combo(); return nc; });
              }
              setFloatingTexts(prev => [...prev, { id: Date.now(), text: `+${(finalReward/1000).toFixed(0)}K 💰`, x: 100, y: playerY, life: 60, color: '#00ff00' }]);
              setCurrentQuote(combo >= 3 ? getQuote('combo') : getQuote('pass'));
          }

          if (newObs.type === 'ice' && !newObs.passed && !isFrozen) {
              const inZone = newObs.x < 130 && newObs.x + OBSTACLE_WIDTH > 30;
              const inGap = playerY + 10 > newObs.gapY && playerY + PLAYER_SIZE - 10 < newObs.gapY + (newObs.dynamicGapSize || GAP_SIZE);
              if (inZone && inGap) {
                  soundManager.ice();
                  setIsFrozen(true);
          ...
          if (!isInvincible && checkCollision(80, playerY, newObs)) {
              if (extraLives > 0) {
                  ...
                  const penalty = rewardDef?.hitType === 'percent' ? Math.floor(capital * Math.abs(rewardDef.hit)) : Math.abs(rewardDef.hit || 0);
                  setCapital(c => {
                      const nc = c - penalty;
                      if (nc <= MIN_CAPITAL) {
                          soundManager.gameOver();
      ...
      // Particles & Texts
      setParticles(prev => prev.filter(p => p.life > 0).map(p => ({ ...p, x: p.x + p.vx, y: p.y + p.vy, life: p.life - 1 })));
      setFloatingTexts(prev => prev.filter(t => t.life > 0).map(t => ({ ...t, y: t.y - 1, life: t.life - 1 })));

      // Score Tick
      if (frameCountRef.current % 30 === 0) {
          setScore(s => {
              const ns = s + (hasDoublePoints ? 20 : 10) * combo;
              if (Math.floor(ns/1000) > Math.floor(s/1000)) soundManager.milestone();
              return ns;
          });
      }

      // Market Mover Events
      if (!activeMarketMover && score > 200 && frameCountRef.current - lastMarketMoverTime > 300 && Math.random() < 0.01) {
          const mover = MARKET_MOVERS[Math.floor(Math.random() * MARKET_MOVERS.length)];
          const quote = mover.quotes[Math.floor(Math.random() * mover.quotes.length)];
      ...
      if (activeMarketMover && marketMoverTimer > 0) {
          setMarketMoverTimer(t => {
              if (t - 1 <= 0) { setActiveMarketMover(null); setMarketEffect(null); }
              return t - 1;
          });
      }
      ...
      if (bossCallActive && frameCountRef.current % 60 === 0) {
          setBossCallTimer(t => {
              if (t - 1 <= 0) { 
                  soundManager.bossFail(); setBossCallActive(false); setProfit(p => Math.floor(p * 0.7)); 
              }
              return t - 1;
          });
          soundManager.bossCall();
      ...
      // Buffett
      if (score >= 3000 && !buffettActive && !buffettCollected && Math.random() < 0.002) {
  ...
  }, [gameState, playerVelocity, playerY, isInvincible, hasSpeedBoost, isFrozen, combo, score, marketEffect, activeMarketMover, bossCallActive, buffettActive, extraLives, hasDoublePoints, marketMoverTimer, checkCollision, applyPowerup]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (showNameInput) {
        if (e.key === 'Enter') {
          if (playerName.trim()) setLeaderboard(prev => [...prev, { name: playerName.trim(), score, character: '😎', date: new Date().toISOString().split('T')[0] }].sort((a,b) => b.score - a.score).slice(0, 10));
          else setLeaderboard(prev => [...prev, { name: 'Anonymous', score, character: '😎', date: new Date().toISOString().split('T')[0] }].sort((a,b) => b.score - a.score).slice(0, 10));
          setShowNameInput(false);
          setShowLeaderboard(true);
        } else if (e.key === 'Backspace') setPlayerName(prev => prev.slice(0, -1));
        else if (e.key.length === 1 && playerName.length < 12) setPlayerName(prev => prev + e.key);
        return;
      }
      if (showLeaderboard) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gradient-to-b from-[#0a0a1a] via-[#1a1a3a] to-[#0a1a2a] overflow-hidden">
      <div className="mb-4 text-center">
        <h1 className="text-3xl md:text-5xl font-bold mb-2 bg-gradient-to-r from-[#00ff88] via-[#00d4ff] to-[#ff00ff] bg-clip-text text-transparent drop-shadow-[0_0_30px_rgba(0,255,136,0.5)] tracking-widest font-mono">
          📈 HODL HERO 🎮
        </h1>
        <p className="text-green-400 text-sm md:text-base tracking-wider font-mono">Lướt Sóng Chứng Khoán - Dễ Hơn Trading Thật!</p>
      </div>
      ...
      <GameRenderer 
        ...
        gracePeriodActive={gameState === 'playing' && frameCountRef.current <= DIFFICULTY.GRACE_PERIOD}
        onJump={jump}
        onCashout={cashout}
      ...
      {gameState === 'menu' && !showLeaderboard && (
         <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <div className="bg-black/80 p-8 rounded-xl border border-green-500 text-center pointer-events-auto backdrop-blur-sm shadow-[0_0_50px_rgba(0,255,136,0.2)]">
                <h2 className="text-4xl text-green-400 mb-6 font-bold tracking-widest font-mono">READY TO TRADE?</h2>
                <div className="space-y-4 w-64">
                    <button onClick={startGame} className="w-full py-4 bg-green-500 rounded-lg font-bold text-[#0a1628] text-xl hover:bg-green-400 transition shadow-[0_0_15px_rgba(0,255,136,0.5)] font-mono">🚀 BẮT ĐẦU</button>
                    <button onClick={() => setShowLeaderboard(true)} className="w-full py-3 bg-yellow-400 rounded-lg font-bold text-[#0a1628] text-lg hover:bg-yellow-300 transition font-mono">🏆 BẢNG XẾP HẠNG</button>
                    <button onClick={() => setSoundEnabled(soundManager.toggle())} className="w-full py-2 bg-gray-800 rounded-lg font-bold text-gray-300 text-sm hover:bg-gray-700 transition border border-gray-600 font-mono">
      ...
      {showLeaderboard && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/95">
              <div className="w-full max-w-md p-6 bg-[#1a1a2e] border border-gray-700 rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.8)]">
                  <h2 className="text-3xl text-yellow-400 text-center mb-6 font-bold tracking-wider font-mono">TOP TRADERS</h2>
                  <div className="space-y-2 mb-6">
                      {leaderboard.map((entry, i) => (
                          <div key={i} className="flex justify-between items-center p-3 bg-white/5 rounded border border-white/10 font-mono">
                              <span className="text-gray-400 w-8">#{i+1}</span>
                              <span className="text-white flex-1 text-left pl-4 text-lg">{entry.character} {entry.name}</span>
                              <span className="text-yellow-400 font-bold text-lg">{entry.score.toLocaleString()}</span>
                          </div>
                      ))}
                  </div>
                  <button onClick={() => setShowLeaderboard(false)} className="w-full py-3 bg-gray-700 text-white rounded-lg font-bold hover:bg-gray-600 transition text-lg font-mono">CLOSE</button>
              </div>
          </div>
      )}
      ...
      {showNameInput && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm">
             <div className="text-center p-8 bg-[#1a1a2e] border border-green-500 rounded-xl shadow-[0_0_50px_rgba(0,255,136,0.3)]">
                 <h2 className="text-4xl text-yellow-400 mb-2 font-bold font-mono">NEW HIGH SCORE!</h2>
                 <p className="text-green-400 text-2xl mb-8 font-mono">{score.toLocaleString()}</p>
                 <input 
                    autoFocus
                    value={playerName}
                    onChange={(e) => setPlayerName(e.target.value.slice(0, 12))}
                    placeholder="ENTER NAME"
                    className="bg-transparent border-b-2 border-green-500 text-center text-3xl text-white outline-none mb-8 w-64 font-bold placeholder-gray-600 font-mono"
                 />
                 <p className="text-gray-500 text-sm animate-pulse font-mono">Press ENTER to save</p>
             </div>
          </div>
      )}

      <div className="mt-4 text-center">
        <p className="text-gray-500 text-xs mb-2 font-mono">Developed for FintechAI</p>
      </div>
    </div>
  );
}